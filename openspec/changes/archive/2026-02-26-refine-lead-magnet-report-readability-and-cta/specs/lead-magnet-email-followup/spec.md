...
The system MUST deliver the full diagnosis report/checklist to the submitted email address after successful diagnosis submission, using concise and readable formatting that drives consultation action.
...
- **AND** the report MUST include personalized segmentation summary, top three priorities, and tool recommendations mapped from response patterns
- **AND** the email MUST include exactly one primary CTA link/button selected by diagnosis grade
- **AND** the email body MUST apply readable paragraph spacing and line-height
