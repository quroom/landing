...
#### Scenario: Production startup validates SMTP configuration contract
- **WHEN** `DEBUG=False` and SMTP delivery backend is selected
- **THEN** startup validation MUST require host, port, sender, and credential contract to be complete
- **AND** the application MUST fail startup when required SMTP contract values are missing

#### Scenario: Pre-deploy readiness command validates SMTP configuration
- **WHEN** deployment readiness command runs before release
- **THEN** the command MUST verify SMTP-related environment contract consistency
- **AND** the command MUST return non-zero exit code if configuration is incomplete or contradictory
...
#### Scenario: Production-safe fallback is blocked in non-debug mode
- **WHEN** `DEBUG=False` and effective email backend resolves to local-only backend
- **THEN** the system MUST reject startup as invalid deployment configuration
- **AND** operators MUST receive explicit guidance to set production email backend values
