...
## Purpose
외국인 개발자 커뮤니티 대기열과 운영 경계 문구를 명확히 정의한다.
...
- **THEN** the page includes a community-sharing section or waitlist opt-in path
- **AND** the path explains the scope of shared information (job search, interview, settlement adaptation)
- **AND** the path states that the live community channel opens after operational threshold is reached
