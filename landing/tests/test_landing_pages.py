...
class LandingPageTests(TestCase):
    ...
    def test_home_page_renders(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, '<html lang="ko">', html=False)
        self.assertContains(response, ">큐룸<", count=2, html=False)
        self.assertContains(
            response,
    ...
    def test_client_analytics_persists_ad_attribution_for_ga4(self) -> None:
        ...
        self.assertIn("quroom_attribution_context", script)
        self.assertIn("URL_ATTRIBUTION_PARAM_MAP", script)
    ...
    def test_outsourcing_checklist_renders_wbs_and_enaradaum_package(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "5대 제외 기준")
        ...
        self.assertContains(response, "린 MVP 기능 명세서 및 표준 견적서")
    ...
    def test_home_engagement_table_links_to_self_serve_wbs_and_no_free_review(
        ...
        self.assertContains(response, "계약 전 5대 제외 기준 &amp; 표준 견적서")
