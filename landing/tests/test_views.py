...
class PreviewReportGroupingTests(SimpleTestCase):
    def _preview_item(
        ...
        *,
        ...
        return {
            ...
            "cta_url": "/#contact",
            "one_action": {"title": "핵심 업무 1개 문서화", "execution": execution},
            "weakest_insight": {
                "label": "업무 흐름 명확성",
                "grade": "B",
                "grade_visible": True,
    ...
    def test_groups_reports_when_action_same_but_weakest_differs(self) -> None:
        first = self._preview_item(title="시나리오 1")
        second = self._preview_item(title="시나리오 2")
        ...
        self.assertEqual(len(grouped), 1)
        self.assertEqual(grouped[0]["scenario_count"], 2)
        self.assertEqual(len(grouped[0]["weakest_insights"]), 2)
