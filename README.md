...
- Search engine registration runbook: `codex-document/search-engine-registration-runbook.md`
...
- `SEARCH_ROBOTS_EXTRA_LINES` (`robots.txt` 하단 추가 라인. 여러 줄 입력 가능. `User-agent`, `Allow`, `Disallow`, `Sitemap`, `Content-Signal`, `#` 주석만 노출)

## Search engine indexing readiness
- Canonical base URL: `https://quroom.kr` (운영 기준)
- Site endpoints:
  - `https://quroom.kr/robots.txt`
  - `https://quroom.kr/sitemap.xml`
- Operator runbook:
  - `codex-document/search-engine-registration-runbook.md`

운영 시 `DJANGO_SITE_BASE_URL`은 canonical 기준 URL과 동일하게 유지하고, 검색엔진 요청에 따라 `SEARCH_ROBOTS_EXTRA_LINES`로 `robots.txt` 하단 문구를 추가합니다. AI 검색/AEO 노출은 열어두되 학습 목적 사용을 제한하려면 `Content-Signal: search=yes,ai-input=yes,ai-train=no`처럼 명시합니다.
